...
const SUB_ADMIN_EMAILS  = []; // Add sub-admin emails here e.g. "subadmin1@gmail.com"
...
let userMembership  = null; // 'VIP' | 'VVIP' | 'VVVIP' | null
...
// ── ── ── ── ── ── ── ── ── ── ── ── ── ── ── ──
//  UI SETUP BASED ON MEMBERSHIP
// ── ── ── ── ── ── ── ── ── ── ── ── ── ── ── ──
function setupUIForMembership() {
  // Show/hide media upload box
  const mediaBox = document.getElementById('mediaUploadBox');
  if (mediaBox) mediaBox.style.display = canPostImages() ? 'block' : 'none';

  // Show/hide audio tab based on VVVIP/admin
  const audioTab = document.querySelector('[onclick*="audio"]');
  if (audioTab) audioTab.style.display = canPostMedia() ? '' : 'none';

  // Show upgrade card if not top tier
  const upgradeCard = document.getElementById('upgradeCard');
  if (upgradeCard) {
    upgradeCard.style.display = (!isAdmin && userMembership !== 'VVVIP') ? 'block' : 'none';
  }
...
window.handleMediaFiles = function(files, type) {
  ...
  [...files].forEach(file => {
    ...
    if (file.size > maxMB[type] * 1024 * 1024) return showToast(`⚠️ File too large (max ${maxMB[type]}MB)`);
...
function renderMediaPreview(type, idx, url, file) {
  if (type === 'photo') {
    ...
    if (!grid) return;
    const div = document.createElement('div');
    div.className = 'preview-item'; div.style.aspectRatio = '16/9';
    ...
    div.innerHTML = `<div class="audio-preview-icon">🎵</div>
...
window.submitPost = async function() {
  ...
  if ((photos.length || videos.length) && !canPostImages()) {
    showToast('⚠️ VVIP or higher required to post images!'); return;
  }
  if ((videos.length || audios.length) && !canPostMedia()) {
    showToast('⚠️ VVVIP required to post videos & audio!'); return;
  }

  if (!text && !photos.length && !videos.length && !audios.length) {
    showToast('✏️ Write something or attach media first!'); return;
  }
...
window.filterVisitorFeed = function(type, btn) {
  document.querySelectorAll('.feed-tab').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
};
...
//  LOAD POSTS
// ── ── ── ── ── ── ── ── ── ── ── ── ── ── ── ──
function loadPosts(feedId = 'postFeed', loadingId = 'feedLoading', emptyId = 'emptyFeed', isVisitor = false) {
  ...
  onSnapshot(q, (snapshot) => {
    ...
    filtered.forEach(post => feed.appendChild(buildPostCard(post, isVisitor)));
...
function buildPostCard(post, isVisitor = false) {
  ...
  const canDelete = isAdmin || isSubAdmin || isOwner;
  ...
  // Member tag
  let memberTagHTML = '';
  if (post.isAdmin) memberTagHTML = '<span class="admin-tag">👑 ADMIN</span>';
  else if (post.membership) {
    const m = post.membership.toLowerCase();
    memberTagHTML = `<span class="post-member-tag ${m}">${post.membership === 'VIP' ? '⭐' : post.membership === 'VVIP' ? '💎' : '👑'} ${post.membership}</span>`;
  }
  ...
  // Footer — visitor sees lock, members see actions
  let footerHTML = '';
  if (isVisitor) {
    footerHTML = `<div class="visitor-lock" onclick="showMembershipModal()">
      🔒 Join to like & comment on this post →
    </div>`;
  } else {
      ...
      <div class="comments-section">
        <div class="comments-list" id="comments-${post.id}">
          ...
          ${canComment() ? `
            ...
            <div class="visitor-lock" onclick="showMembershipModal()">🔒 Join to comment →</div>`}
        </div>
      </div>`;
  }
...
window.toggleComments = function(postId) {
  const el = document.getElementById('comments-' + postId); if (el) el.classList.toggle('open');
};

window.addComment = async function(postId) {
  if (!currentUser || !canComment()) { showToast('⚠️ Membership required to comment!'); return; }
...
onAuthStateChanged(auth, async user => {
  ...
  if (user) {
    ...
    } catch(e) { console.warn('Could not fetch membership', e); }

    // Admin & sub-admin always get full access
    ...
    if (adminBadge) adminBadge.style.display = (isAdmin || isSubAdmin) ? 'inline-block' : 'none';
    if (adminBadge && isSubAdmin && !isAdmin) adminBadge.textContent = '🛡️ SUB ADMIN';

    // Admin notification bar
    const notifBar = document.getElementById('adminNotifBar');
    if (notifBar) notifBar.style.display = (isAdmin || isSubAdmin) ? 'block' : 'none';
    ...
    // Load posts in read-only visitor mode
    loadPosts('visitorPostFeed', 'visitorFeedLoading', 'visitorEmptyFeed', true);
  }
});
